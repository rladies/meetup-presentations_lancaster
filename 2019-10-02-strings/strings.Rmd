...
Using `stringr::sentences`, answer the following.

1. How many sentences contain the word "green"?
2. Which sentence has the most characters? Hint: look at the help for `which.max()`
3. Convert all the sentences to uppercase.
4. What does `str_count()` do? 
5. `longword = "Llanfairpwllgwyngyllgogerychwyrndrobwllllantysiliogogogoch"` Use `str_length()` and `str_sub()` to extract the middle character(s) from `longword`. What will you do if the string has an even number of characters?
6. Replace all fullstops with exclaimation marks. Hint: you'll need to use `\\.` and `\\!`
